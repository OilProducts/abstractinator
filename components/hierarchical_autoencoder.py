...
#
#                 # Which compressor step aligns with this expander?
#                 comp_step = compression_results["steps"][self.num_levels - 1 - i]
#
#                 # 1) Build a segment-level KV mask: (B, Lkv), True = pad
#                 kv_mask = None
#                 if getattr(comp_step, "valid_mask", None) is not None:
#                     kv_mask = ~comp_step.valid_mask  # invert: True==pad
#
...
#                     src_key_padding_mask=kv_mask,  # ← segment-level mask here
#                     tgt_key_padding_mask=tgt_kpm_for_this_level,  # child token KPM unchanged
...
#             self,
#             all_logits: list[list[torch.Tensor]],  # per level: list of [B, L, K] per stage
#             targets: list[torch.Tensor],  # per level: (B, L) composed indices at that level
#             target_kpms: list[torch.Tensor | None]
#     ) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
#         """
#         Computes factorized CE over stages for each level's expander output.
#         Ignores PAD in `target_kpms` and ignores positions where the composed
#         target is a 'special' (EOS/EOP/PAD) according to that level's codec.
#         """
...
#             stage_logits_list = all_logits[i]  # list of tensors, len == depth
#             # Get the codec the expander used to produce those logits
#             codec = self.expanders[i].lo_codec  # ComposedIndexCodec for that level
#             depth = len(stage_logits_list)
#
#             # Decompose the *composed* targets into per-stage digits and specials mask
#             digits_i, specials_mask = codec.decompose(targets[i])  # digits_i: list[(B, L)], specials_mask: (B, L) bool
#
#             # Valid positions: not PAD and not special tokens
#             kpm_i = target_kpms[i]
#             valid = (~kpm_i) if kpm_i is not None else torch.ones_like(targets[i], dtype=torch.bool)
#             valid = valid & (~specials_mask)
#
#             # If nothing to train on (possible early in training), skip safely
#             denom = valid.sum().clamp(min=1)
#
#             # Sum CE over stages (you may /depth if you want an average per-position)
#             ce_sum = torch.tensor(0.0, device=device, dtype=torch.float32)
#             for s in range(depth):
#                 logits_s = stage_logits_list[s]  # (B, L, K_s)
#                 # Flatten valid positions
#                 if valid.any():
#                     ce_s = F.cross_entropy(
#                         logits_s[valid],  # (N, K_s)
#                         digits_i[s][valid],  # (N,)
#                         reduction='sum'
#                     )
#                     ce_sum = ce_sum + ce_s
#
#             level_loss = ce_sum / denom
#             total = total + level_loss
#
#             # Optional: surface per-level metric names similar to before
#             expander_k_lo = getattr(self.expanders[i], "K_lo", None)
#             if expander_k_lo == self.initial_vocab_size:
#                 suffix = "_bytes"
#             else:
#                 reconstructed_code_level_idx = self.num_levels - 2 - i
#                 suffix = f"_codesL{reconstructed_code_level_idx}" if reconstructed_code_level_idx >= 0 else "_intermediate"
#             details[f"reconstruction_to{suffix}"] = level_loss
#
#         avg = total / self.num_levels if self.num_levels > 0 else torch.tensor(0.0, device=device, dtype=torch.float32)
#         return avg, details
#
