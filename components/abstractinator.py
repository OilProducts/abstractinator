...
class Abstractinator(nn.Module):
    ...
    def __init__(
        ...
        # ---- 2) Expander
        # hi side is ALWAYS thsi level's VQ
        hi_vq = self.compressor.vq
        ...
        self.expander = DecoderOnlyExpanderRVQ(
            lo_vq=lo_vq,
            hi_vq=hi_vq,                            # None at bottom level; non-None for upper levels
            K_lo=K_lo,
            D=cfg.D,
            N_dec=cfg.d_layers,
            H=cfg.d_heads,
    ...
    def compute_code_loss(
        ...
        digit_loss_sum = 0.0
        denom = 0
        with torch.autocast(device_type=codes_lo.device.type, enabled=False) if hasattr(torch, 'autocast') else _nullcontext():
    ...
    def forward(
        ...
        hi_memory: Optional[torch.Tensor] = None,           # (B, S_hi, D) continuous or (B, S_hi) discrete if hi_vq used in expander
